/*
 * ParkIRC Exit Gate Controller
 * 
 * Controls the exit gate for a parking system
...
const int PRINTER_TRIGGER_PIN = 6; // Pin to trigger external receipt printer (optional)
...
void setup() {
  ...
  Serial.println("Exit Gate Controller initialized");
}
...
void checkSerialCommands() {
  if (Serial.available() > 0) {
    ...
    if (command == "OPEN_GATE") {
      ...
      // Extract receipt data from command (format: PRINTV:DATA_TO_PRINT)
      receiptData = command.substring(7);
      triggerPrinter();
    }
  }
}
...
void checkVehicleSensor() {
  ...
  if (currentSensorState != vehicleDetected) {
    ...
    if (vehicleDetected) {
      ...
      // For exit gates, we want to capture the vehicle image too
      triggerCamera();
      
      // For exit gates, we may want to automatically open when payment is validated
      // This would typically be controlled by the desktop app sending an OPEN_GATE command
    } else {
      // Vehicle has left sensor area
      Serial.println("VEHICLE_LEFT:EXIT");
    }
  }
}
...
void checkButton() {
  ...
  if ((millis() - lastDebounceTime) > DEBOUNCE_DELAY) {
    if (reading != buttonState) {
      ...
      if (buttonState == LOW) {
        Serial.println("BUTTON_PRESSED:EXIT");
        toggleGate();
      }
    }
  }
...
void triggerPrinter() {
  ...
  Serial.println("PRINTING_RECEIPT:" + receiptData);
}
